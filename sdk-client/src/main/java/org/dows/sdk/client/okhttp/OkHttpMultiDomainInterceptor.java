...
import org.dows.sdk.client.http.HttpConstant;
...
public class OkHttpMultiDomainInterceptor implements Interceptor {
    ...
    private boolean shouldRetry(Request request) {
        return HttpConstant.PRIMARY_API_DOMAIN.contains(request.url().host());
    }

    private Request modifyRequestForRetry(Request originalRequest) {
        HttpUrl.Builder urlBuilder = originalRequest.url().newBuilder();
        urlBuilder.host(HttpConstant.SECONDARY_API_DOMAIN);

        Request.Builder reqBuilder = originalRequest.newBuilder();
        reqBuilder.url(urlBuilder.build());
        reqBuilder.header(HttpConstant.USER_AGENT, originalRequest.header(HttpConstant.USER_AGENT) + " (Retried-V1)");
